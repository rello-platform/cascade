 ...
 */
export async function pushToSpokes(tenantId, resourceType, payload, options) {
    ...
    const { spokes, excludeSpokeSlug, onlySpokeSlug, timeoutMs = DEFAULT_TIMEOUT_MS, force, } = options;
    ...
    const results = await Promise.all(targets.map((spoke) => pushOne(spoke, resourceType, body, timeoutMs)));
    const synced = results.filter((r) => r.ok).length;
    const failed = results.length - synced;
    const errors = results.filter((r) => !r.ok).map((r) => r.error);
    return { status: "ok", synced, failed, errors };
}
function isUsable(spoke, excludeSlug, onlySlug) {
    if (!spoke || typeof spoke !== "object")
        return false;
    if (typeof spoke.slug !== "string" || spoke.slug.length === 0)
        return false;
    if (typeof spoke.baseUrl !== "string" || spoke.baseUrl.length === 0)
        return false;
    if (typeof spoke.apiKey !== "string" || spoke.apiKey.length === 0)
        return false;
    if (excludeSlug && spoke.slug === excludeSlug)
        return false;
    if (onlySlug && spoke.slug !== onlySlug)
        return false;
    return true;
}
async function pushOne(spoke, resourceType, body, timeoutMs) {
...
//# sourceMappingURL=push-to-spokes.js.map